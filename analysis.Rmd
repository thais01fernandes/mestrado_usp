...
dados_normalizados <- banco_completo %>% select(cod_munic,
                          tarifa_zero,
                          populacao, 
  ...
  mutate(secretaria_exclusiva = case_when(secretaria_exclusiva %in% c("Não informou", "Não possui estrutura", "Recusa",
   ...
   mutate(possui_plano_de_mobilidade_urbana = case_when(
    ...
    mutate(escolaridade_do_a_titular_do_orgao_gestor_2 = case_when(
    escolaridade_do_a_titular_do_orgao_gestor %in% c("Ensino superior completo", "Mestrado", "Especialização") ~ 1,
    escolaridade_do_a_titular_do_orgao_gestor %in% c("-", "Recusa", "Ensino médio (2º Grau) completo", "Ensino superior incompleto", "Ensino fundamental (1º Grau) incompleto","Ensino médio (2º Grau) incompleto", "Ensino fundamental (1º Grau) completo", "Não informou", "Doutorado", "Sem titular") ~ 0,
    TRUE ~ as.numeric(escolaridade_do_a_titular_do_orgao_gestor))) %>% 
     pivot_wider(names_from = escolaridade_do_a_titular_do_orgao_gestor, values_from = escolaridade_do_a_titular_do_orgao_gestor_2) %>% 
     mutate(prosperidade_social_2= case_when(
     prosperidade_social %in% c("Muito Alto", "Alto") ~ 1,
     prosperidade_social %in% c("Médio", "Baixo", "Muito Baixo") ~ 0, 
     TRUE ~ as.numeric(prosperidade_social))) %>% 
   pivot_wider(names_from = prosperidade_social, values_from = prosperidade_social_2) %>% 
   select(2,4,5,6, 7, 8,11, 12, 13, 17, 20, 23,28,29) %>% 
   mutate(`Ensino superior completo` = replace_na(`Ensino superior completo`, 0)) %>% 
   mutate(Mestrado = replace_na(Mestrado, 0)) %>% 
   mutate(`Muito Alto` = replace_na(Especialização, 0)) %>% 
   mutate(Alto = replace_na(Alto, 0)) %>% 
   mutate(Especialização = replace_na(Especialização, 0)) %>% 
   mutate(conselho_municipal_de_transporte_existencia = replace_na(conselho_municipal_de_transporte_existencia, 0)) %>% 
   mutate(fundo_municipal_de_transporte_existencia = replace_na(fundo_municipal_de_transporte_existencia, 0)) %>% 
   rename(plano_transporte = plano_municipal_de_transporte_existencia, 
         plano_mobilidade = possui_plano_de_mobilidade_urbana, 
         conselho_transp = conselho_municipal_de_transporte_existencia, 
         fundo_transp = fundo_municipal_de_transporte_existencia, 
         Ensino_superior =  `Ensino superior completo`, 
         Especializacao = Especialização,
         prosp_muito_alta = `Muito Alto`,
         prosp_alta = Alto, 
         pib = pib_normalizado,
         populacao = populacao_normalizado,
         qt_concursados = estatutarios_normalizado, 
         secretaria_excl = secretaria_exclusiva) %>% 
  mutate(qt_concursados = replace( qt_concursados, is.infinite(qt_concursados), 0))
...
dados_normalizados %>% 
  pivot_longer(cols = c("pib_normalizado", "populacao_normalizado", "estatutarios_normalizado")) %>% 
  ggplot() + 
  geom_boxplot(aes(x = name, y = value, fill = name), colour="black", alpha = 0.9) +
  theme_light() +
  ggtitle("") +
  xlab("") +
  ylab("")
...
matriz_correlacao <- cor(dados_normalizados)

ggplot(data = as.data.frame(as.table(matriz_correlacao)), aes(Var1, Var2, fill = Freq)) +
  geom_tile() +
  scale_fill_gradient(low ="#faccff"  , high = "#250b65") +
  theme_minimal() +
...
dados_normalizados %>%
  ggplot(aes(x = prosp_muito_alta, y = Especializacao))+
  geom_point(col = 'darkblue')+
  geom_smooth(method = 'lm', se = F, col = 'red')+
  theme_bw()+
  labs(title = 'Correlação entre Prosperidade Alta da Cidade e Especialização 
       dos Gestores de Política Urbana')
```
