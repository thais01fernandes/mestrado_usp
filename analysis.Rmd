...
regiao_sudeste <- banco_completo %>% 
  ...
  ggplot() + 
  geom_point(aes(x = ano_implementacao, y = populacao_estimada, size = populacao_estimada), color = "#579296", alpha = 0.7) + 
  # geom_text_repel(aes(x = ano_implementacao, y = populacao_estimada, label = scales::dollar(prefix = " ", round(populacao_estimada, 1))), size = 3, hjust = 0.5, vjust = 1.5,point.padding = 0.5) + 
   theme_light() +
  ggtitle("Tamanho da população e ano de implementação de cada cidade 
          Região: Sudeste") + 
...
options(scipen = 999)
  ...
  ggplot() + 
  geom_point(aes(x = ano_implementacao, y = populacao_estimada, color = `Região:`), alpha = 0.5, size = 5) + 
 theme_light() +
  ggtitle("Tamanho da população e ano de implementação de cada cidade 
          Todas as Regiões") +
  xlab("") +
...
banco_completo_1 %>% 
  ...
  color="black", size=3) + 
  annotate(
  "label", x=2020, y=10, label="Pandemia de Covid-19",
  color="black", size=3)
