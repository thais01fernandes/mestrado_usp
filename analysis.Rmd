...
file2 <- "https://raw.githubusercontent.com/thais01fernandes/mestrado_usp/main/dados/geo_ufs_2"
geo_ufs_2 <- read_delim(file2, delim = ",", 
                        locale = locale(encoding='latin1')) 


file3 <- "https://raw.githubusercontent.com/thais01fernandes/mestrado_usp/main/dados/muni_geobr_3"
muni_geobr_3 <- read_delim(file3, delim = ",", 
                        locale = locale(encoding='latin1'))
...
ffpt_orgao <- banco_completo %>% 
  mutate(caracterizacao_do_orgao_gestor = case_when(caracterizacao_do_orgao_gestor =="Não informou" ~ "Não possui ou não informou",
                                                      caracterizacao_do_orgao_gestor == "Não possui estrutura" ~ "Não possui ou não informou", 
                                                      caracterizacao_do_orgao_gestor == "Recusa" ~ "Não possui ou não informou",
...
banco_completo %>% 
  ...
  kbl(col.names = c("Caracterização do orgão gestor", "Cidades sem Tarifa Zero", "Cidades com Tarifa Zero"), caption = "Tabela 3: Caracterização do Orgão Gestor") %>%
...
kbl(col.names = c("Escolaridade", "Cidades sem Tarifa Zero", "Cidades com Tarifa Zero"), caption = "Tabela 4: Escolaridade do Titular do Orgão Gestor") %>%
kable_classic(full_width = F , html_font = "Cambria") %>% 
footnote(general = "IBGE, 2021 ",
           general_title = "Fonte: ", number_title = "Type I: ",
           footnote_as_chunk = T, title_format = c("italic")
...
banco_completo %>% 
  ...
  adorn_pct_formatting(digits = 0) %>% 
kbl(col.names = c("Existe Conselho na Cidade?", "Cidades sem Tarifa Zero", "Cidades com Tarifa Zero"), caption = "Tabela 5: Existência de Conselho Municipal de Transporte") %>%
kable_classic(full_width = F , html_font = "Cambria") %>% 
footnote(general = "IBGE, 2021 ",
           general_title = "Fonte: ", number_title = "Type I: ",
           footnote_as_chunk = T, title_format = c("italic")
...
banco_completo %>%
  ...
  adorn_pct_formatting(digits = 0) %>% 
kbl(col.names = c("Existe Fundo Municipal?", "Cidades sem Tarifa Zero", "Cidades com Tarifa Zero"), caption = "Tabela 6: Existência de Fundo Municipal de Transporte") %>%
kable_classic(full_width = F, html_font = "Cambria") %>% 
footnote(general = "IBGE, 2021 ",
           general_title = "Fonte: ", number_title = "Type I: ",
           footnote_as_chunk = T, title_format = c("italic")
...
banco_completo %>% 
  ...
  adorn_pct_formatting(digits = 0) %>% 
kbl(col.names = c("Existe Plano de Mobilidade Urbana?", "Cidades sem Tarifa Zero", "Cidades com Tarifa Zero"), caption = "Tabela 6: Existência de Plano de Mobilidade Urbana") %>%
kable_classic(full_width = F, html_font = "Cambria") %>% 
footnote(general = "IBGE, 2021 ",
           general_title = "Fonte: ", number_title = "Type I: ",
           footnote_as_chunk = T, title_format = c("italic")
...
banco_completo %>% 
  ...
  adorn_pct_formatting(digits = 0) %>% 
  mutate(pct_ffpt = gsub(pattern = "-",replacement = "0%", pct_ffpt)) %>% 
kbl(col.names = c("Índice de Prosperidade Social", "Cidades sem Tarifa Zero", "Cidades com Tarifa Zero"), caption = "Tabela 6: Índice de Prosperidade Social") %>%
kable_classic(full_width = F, html_font = "Cambria") %>% 
...
**7° premissa:** Cidades que implementam políticas redistributivas dispõem de mais recursos e melhor capacidade administrativa. Verificar a quantidade de funcionários estatutários dos municípios - Carreira estável e recrutamento baseado no mérito produz capacidade estatal para adoção de políticas urbanas
