...
votacao_2000 <- read_delim("votacao_2000.csv", 
    delim = ";", escape_double = FALSE, locale = locale(encoding = "Latin1", 
        asciify = TRUE), trim_ws = TRUE) %>% filter(DS_CARGO == "Prefeito")


votacao_2004 <- read_delim("votacao_2004.csv", 
    delim = ";", escape_double = FALSE, locale = locale(encoding = "Latin1", 
        asciify = TRUE), trim_ws = TRUE) %>% filter(DS_CARGO == "Prefeito")


votacao_2008 <- read_delim("votacao_2008.csv", 
    delim = ";", escape_double = FALSE, locale = locale(encoding = "Latin1", 
        asciify = TRUE), trim_ws = TRUE) %>% filter(DS_CARGO == "Prefeito")

votacao_2012 <- read_delim("votacao_2012.csv", 
    delim = ";", escape_double = FALSE, locale = locale(encoding = "Latin1", 
        asciify = TRUE), trim_ws = TRUE) %>% filter(DS_CARGO == "Prefeito")


votacao_2016 <- read_delim("votacao_2016.csv", 
    delim = ";", escape_double = FALSE, locale = locale(encoding = "Latin1", 
        asciify = TRUE), trim_ws = TRUE) %>% filter(DS_CARGO == "Prefeito")


votacao_2020 <- read_delim("votacao_2020.csv", 
    delim = ";", escape_double = FALSE, locale = locale(encoding = "Latin1", 
        asciify = TRUE), trim_ws = TRUE) %>% filter(DS_CARGO == "Prefeito")  
...
cidades_tarifa_zero <- banco_completo %>% 
  filter(tarifa_zero == "sim") %>% 
  select(cod_munic, tarifa_zero) %>% 
  rename(CD_MUNICIPIO = cod_munic) %>% 
  mutate(CD_MUNICIPIO = as.character(CD_MUNICIPIO)) %>% 
  mutate(CD_MUNICIPIO = str_sub(CD_MUNICIPIO, start = 3))

votacao_2008 %>% 
left_join(cidades_tarifa_zero, by = "CD_MUNICIPIO") %>% 
  filter(tarifa_zero == "sim")
  
# Checando quantos muncípios tem em cada base 

votacao_2020 %>% select(CD_MUNICIPIO) %>% 
  filter(CD_MUNICIPIO == "50308")
