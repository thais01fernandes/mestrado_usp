...
```{r Gráfico GINI e Theil 1, fig.align = 'center', out.width = "60%", dpi=300}
...
banco_completo %>% 
  ...
  mutate(name = case_when(
    ...
    name == "theil" ~ "Theil-L")) %>% 
    ggplot() +
    geom_boxplot(aes(x = tarifa_zero, y = value, fill = tarifa_zero), colour="black", alpha = 0.6) + 
  facet_wrap(~name) +
  theme_light() +
  xlab("") +
  ylab("") +
  scale_fill_manual(name = "Tarifa Zero?", values = c("#00688b", "#954182"), labels = c("Não", "Sim"))+ 
  theme_light() + 
    theme(plot.title = element_text(family = "serif", size = 12, face = "bold", hjust = 0.5, colour = "black"),
